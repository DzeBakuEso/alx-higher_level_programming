...
class Rectangle:
    ...
    def __str__(self):
        """This returns string repres of the recgle using the'#'character."""
        if self.__width == 0 or self.__height == 0:
            return ""
        return (("#" * self.__width + "\n") * self.__height).rstrip()

    def __repr__(self):
        """This returns a string that recreate a new instnce using eval()."""
        return "Rectangle({}, {})".format(self.__width, self.__height)

    def __del__(self):
        """This prints a message when the rectangle instance is deleted."""
        print("Bye rectangle...")
